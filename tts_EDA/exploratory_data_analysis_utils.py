...
def plot_computed_dists(dists, 
    ...
    for i, idx in enumerate(numerator_dist_idxs):
        try:
            lines = ax2.plot(bins[idx][:-1],ns[idx] / ns[denominator_dist_idxs[i]], 
                    alpha=0.8,label='{} to {}'.format(labels[idx],labels[denominator_dist_idxs[i]]),)
...
def compute_to_wall(position, angle):
    """
    compute_to_wall(position, angle)
    
    Purpose : Calculate distance from event origin to IWCD wall along particle trajectory.
    
    Args: 
        position            ... array of [x, y, z] co-ordinates of event origin
        angle               ... array of [theta, phi] angles of departure
    author: Calum Macdonald
    May 2020
    """
    cylinder_half_height = 300 # Old value 521
    R = cylinder_radius = 400 # Old value 371

    x = position[0]
    y = position[2]
    z = position[1]

    theta = angle[0]    # polar angle
    phi = angle[1]      # azimuthal angle
    ...
    # distance is to instersection with tank radius
    to_wall = (-b + np.sqrt(b*b-a*c)) / a

    if abs(z + to_wall*dir_z) > cylinder_half_height:
        # distance to endcap is nearer than to tank radius
        to_wall = cylinder_half_height/abs(dir_z) - z/dir_z

    return to_wall

def compute_d_wall(position):
    """
    ...
    # Solve for nearest solution

    dist_to_barrel = cylinder_radius - np.sqrt(x**2 + y**2)
    dist_to_endcap = cylinder_half_height - np.abs(z)
...
def compute_event_hit_type(position, angle):
    ...
    # distance is to instersection with tank radius
    to_wall = (-b + np.sqrt(b*b-a*c)) / a
    hit_type = hit_type_dict['barrel']

    if abs(z + to_wall*dir_z) > cylinder_half_height:
        # distance to endcap is nearer than to tank radius
        to_wall = cylinder_half_height/abs(dir_z) - z/dir_z
        hit_type = hit_type_dict['endcap']
